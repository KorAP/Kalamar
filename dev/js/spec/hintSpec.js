...
define(['hint', 'hint/input', 'hint/contextanalyzer', 'hint/menu', 'hint/item'], function (hintClass, inputClass, contextClass, menuClass, menuItemClass) {
  ...
  describe('KorAP.Hint', function () {
    ...
    it('should support prefix', function () {
      ...
      console.log("click: ",hint.active()._prefix.element().click);
      console.log("input: ",input); //usually, the input.value should be overwritten, but it isnt.
      const m = hint.active()._prefix.menu();
      console.log("menu :",m);
      const value = hint.active()._prefix.value();
      console.log("value: ",value); //This works. 
      const h = m.hint();
      console.log("hint: ",h);
      //hint.active()._prefix.onclick(10);// this throwing "e.halt is not a function" shows that this function has been everwritten correctly.
      hint.active()._prefix.element().click();
      
      
      hint.inputField().insert("cor");
