...
define([ 'view', 'vc/statistic' ], function(viewClass, statClass) {
  ...
  return {
     ...
     */
    checkGrayingStatistic : function(doct){
     ...
     if(newString) {       
       //if newString === oldString vc is not changed
       if(newString !== oldString){      
         // if part of the vc is deleted, vc (but not docGroupRef or doc) is updated
         if(doctype.ldType() === null){
           if(oldString.indexOf(newString) != -1 ){
             this.grayingStat();  
           }
         } 
         if( (doctype.ldType() === "doc"  &&  doctype.value() !== undefined ) || 
             (doctype.ldType() === "docGroupRef"  &&  doctype.ref() !== undefined )) {
           this.grayingStat();  
         }
       }  
     }
