...
define([
  ...
  domReady(function (event) {
     ...
     */
    d.querySelectorAll('#notifications div.notify').forEach(function (e) {
    ...
    if (input) {
      ...
      if (
        KorAP.koralQuery !== undefined &&
        (KorAP.koralQuery['collection'] || KorAP.koralQuery['corpus'])
      ) {
        try {
          vc.fromJson(
            KorAP.koralQuery['collection'] || KorAP.koralQuery['corpus']
     ...
     */
    var matchElements = d.querySelectorAll('#search > ol > li');
    ...
    matchElements.forEach(function (e) {
      ...
      // Define class for inactive elements
        ...
        e.addEventListener('click', function (e) {
          if (this._match !== undefined) this._match.open();
          else {
            // lazyLoad
        ...
        e.addEventListener('keydown', function (e) {
          ...
          switch (code) {
            case 32:
              if (this._match !== undefined) this._match.toggle();
              else {
                // lazyLoad
                matchClass.create(this).open();
              }
    ...
    if (qlField !== null) {
      KorAP.QLmenu = selectMenuClass
        .create(d.getElementById('ql-field').parentNode)
        .limit(10);
    }
     ...
     */
    if (matchElements.length > 0) resultPanel.addAlignAction();
    ...
    if (vcname) {
      ...
      vc.onOpen = function () {
        ...
        if (!view.firstChild) view.appendChild(this.element());
    ...
    if (obj.hint === undefined) obj.hint = hintClass.create();
