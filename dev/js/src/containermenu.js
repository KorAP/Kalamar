...
define([
  ...
  return {
    ...
    create : function (list, params, containerList) {
      ...
      //Need to get rid of old prefix element, to be able to recreate it as an li element
      //TODO: Better Idea Nils? New span scss subclass/...
      delete obj._prefix._el;
      obj._prefix._el = document.createElement('li');
      if (obj._prefix["onclick"] !== undefined) obj._prefix._el["onclick"] = obj._prefix.onclick.bind(obj._prefix);
    ...
    show : function (active) {
      ...
      if (!t._initList()) {
        ...
        t._container._el.classList.add('visible');
    ...
    hide : function () { //TODO need changes???
      if (!this.dontHide) {
        ...
        this._container._el.classList.remove('visible');
      }
      // this._el.blur();
    },
    ...
    prev : function () {
      ...
      if (newItem === undefined) { //too far
        ...
        if (cLLength !== 0){ //actually makes sense to next
          ...
          if (notInContainerAnyMore) { //oh, next one (should not happen, because cLLength is now liveLength)
            //console.log("Container done: To bottom");
      ...
      // The previous element is before the view - roll up
        ...
        //console.log("prev is: ",t.position);
        t.screen(t.position);
      }
