...
define([
  ...
  return {
    ...
    next : function () {
      ...
      const cLength = c.liveLength();
      // No list
      if (t.liveLength()===0){
        if (cLength === 0) return;
        notInContainerAnyMore = c.next();
        if (notInContainerAnyMore && cLength>1) {
          c.next();
        }
        return;
      }
      if (!c.active() && t.position!==-1) {t.liveItem(t.position).active(false);}
      t.position++;
      let newItem = t.liveItem(t.position); //progress
      if (newItem === undefined) { //too far
        if (cLength !== 0){ //actually makes sense to next
          notInContainerAnyMore = t._container.next(); //activate container
          if (notInContainerAnyMore) { //oh, next one
            t.liveItem(0).active(true);
            t.position = 0;
            t._showItems(0);
          }
        } else {
