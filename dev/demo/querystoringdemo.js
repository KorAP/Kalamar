...
require(['plugin/server','panel/query', 'hint', 'hint/foundries/cnx','lib/domReady','api'], function (pluginClass, queryPanelClass, hintClass, hintArray, domReady) {
  ...
  // used in hint-menus. You must also overwrite hints item with the one provided in querystoringdemo_newhintitem.js

  //Due to this the new buttons appear WITHIN the hint.menu("-"), so where we usually have the query options.
  // We recommend using querystoringdemoVariant2 instead, that version has prettier code and also the query options
  // are displayed in the container (below the regular menu) instead of inside the menu. This looks a lot better and
  // makes more sense.
  ...
  // overwrite the onclick function in hint/item.js with the one provided in hint/querystoringitem.js. The buttons need to have be stored
  ...
  domReady(function() {
    ...
    KorAP.triggerDeleteModeButton = { // Whenever used is at location 1
      ...
      onclick : function (e) {
        ...
        if (this._menu.hint()._hintItemMode === "REGULAR") {
          ...
          this.menu().container().item(0).initContent("Cancel Deletion");
          ...
          this.menu().container().item(0).initContent(); //"Back"
    ...
    KorAP.Hint.readQueries = function (JSONList) {
      ...
      this.menu("-").readItems(JSONListToQueryTemplates(JSONList));
      ...
      console.log(this.menu("-").container()._cItemPrefix);
      this.menu("-").show();
    }.bind(KorAP.Hint);
