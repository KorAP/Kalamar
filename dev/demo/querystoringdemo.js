...
require(['plugin/server','panel/query', 'hint','hint/foundries/cnx','lib/domReady','api'], function (pluginClass, queryPanelClass, hintClass, hintArray, domReady) {
  ...
  domReady(function() {
    ...
    KorAP.Hint.readQueries = function (JSONList) {
      ...
      h.menu("-").readItems(queryItemTemplates);
      h._queryReadMode = "READ"; //alternative: "DELETE"
      h._deleteTheseQueries = [];
      h.menu("-").container().removeItemByIndex(0); //Save Button
      h.menu("-").container().removeItemByIndex(0); //Display Button
      h.menu("-").container().addItem(KorAP.restoreMinusMenuButton);
      h.menu("-").container().addItem(KorAP.triggerDeleteModeButton);
    };
    ...
    KorAP.Hint.unReadQueries.bind(KorAP.Hint); //Maybe not necessary
